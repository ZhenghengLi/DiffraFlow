...
__global__ void energy_sum_kernel(
    diffraflow::ImageDataField* image_data_device, double* sum_device, int* count_device) {
    ...
    for (int h_asic = 1; h_asic < 63; h_asic++) {
        for (int w_asic = 0; w_asic < 64; w_asic++) {
            sum_local += image_data_device->pixel_data[mod][row * 64 + h_asic][col * 64 + w_asic];
            count_local++;
        }
    }
...
__global__ void mean_square_sum_kernel(
    float mean, diffraflow::ImageDataField* image_data_device, double* sum_device, int* count_device) {
    int mod = blockIdx.x;  // module
    int row = threadIdx.x; // ASIC row
    int col = threadIdx.y; // ASIC column
    ...
    for (int h_asic = 1; h_asic < 63; h_asic++) {
        for (int w_asic = 0; w_asic < 64; w_asic++) {
            double residual = image_data_device->pixel_data[mod][row * 64 + h_asic][col * 64 + w_asic] - mean;
...
void diffraflow::FeatureExtraction::global_mean_rms_gpu(cudaStream_t stream, double* sum_device, int* count_device,
    ...
    // mean
    energy_sum_kernel<<<MOD_CNT, dim3(8, 2), 0, stream>>>(image_data_device, sum_device, count_device);
    ...
    // rms
    mean_square_sum_kernel<<<MOD_CNT, dim3(8, 2), 0, stream>>>(mean, image_data_device, sum_device, count_device);
