...
diffraflow::AggSenderConsumer::AggSenderConsumer(AggMetrics* metrics) : AggBaseConsumer("sender-consumer") {
...
void diffraflow::AggSenderConsumer::process_message_(const pulsar::Message& message) {
    ...
    if (err) {
        ...
        aggregated_metrics_->set_metrics("sender", metrics_key, metrics_json_simplified);
    }
}

json::value diffraflow::AggSenderConsumer::simplify_metrics_(const json::value& metrics_json) {
