...
bool diffraflow::ImageFileHDF5W::append(const ImageData& image_data) {
    ...
    if (buffer_pos_ < buffer_size_) {
        ImageDataHDF5::convert_image(image_data, image_buffer_[buffer_pos_]);
        buffer_pos_++;
    }
    if (buffer_pos_ == buffer_size_) {
        return flush();
...
bool diffraflow::ImageFileHDF5W::flush() {
    ...
    try {
        ...
        imgdat_dset_.write(image_buffer_, image_data_hdf5, memory_space, file_space);
        // SWMR flush
        H5Dflush(imgdat_dset_id_);
        // advance position
        imgdat_dset_pos_ += buffer_pos_;
        buffer_pos_ = 0;
    } catch (H5::Exception& e) {
