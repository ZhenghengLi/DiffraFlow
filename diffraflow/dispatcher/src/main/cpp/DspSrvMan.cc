...
void diffraflow::DspSrvMan::start_run() {
    ...
    // create senders
    if (create_senders_(combiner_address_file_.c_str(), config_obj_->dispatcher_id,
        config_obj_->compress_method, config_obj_->compress_level)) {
...
bool diffraflow::DspSrvMan::create_senders_(const char* address_list_fn, int dispatcher_id,
    DspSender::CompressMethod compress_method, int compress_level) {
    ...
    for (size_t i = 0; i < addr_vec.size(); i++) {
        sender_arr_[i] = new DspSender(addr_vec[i].first, addr_vec[i].second,
            dispatcher_id, compress_method, compress_level);
